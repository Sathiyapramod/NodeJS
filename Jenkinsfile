     ...
     stages {
          stage('initial'){
               steps {
                    ...
                    script {
                         def qualityGateStatus = sh(script: 'sonar-scanner -Dsonar.login=YOUR_SONAR_LOGIN_TOKEN -Dsonar.host.url=https://sonarcloud.io -Dsonar.organization=YOUR_SONAR_ORG -Dsonar.projectKey=YOUR_PROJECT_KEY -Dsonar.projectName=YOUR_PROJECT_NAME -Dsonar.sources=. -Dsonar.language=java -Dsonar.exclusions=**/*.txt -Dsonar.showProfiling=true -Dsonar.showProfiling=true | awk -F '"status":"|"' \'{print $4}\'', returnStdout: true).trim()
